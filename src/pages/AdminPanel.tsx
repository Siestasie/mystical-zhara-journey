...
const AdminPanel = () => {
  ...
  useEffect(() => {
    fetch("https://85.192.30.34:3000/api/products")
      .then((res) => res.json())
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://85.192.30.34:3000/api/orders/all");
      if (response.ok) {
  ...
  const updateOrderStatus = async (orderId: number, newStatus: string) => {
    try {
      ...
      const response = await fetch(`https://85.192.30.34:3000/api/orders/${orderId}/status`, {
        method: "PUT",
      ...
      if (response.ok) {
        ...
        if (newStatus === 'completed' || newStatus === 'cancelled') {
          ...
          if (activeTab === "history") {
            ...
            const completedResponse = await fetch("https://85.192.30.34:3000/api/orders/all");
  ...
  const applyDiscount = async () => {
    ...
    try {
        setLoading(true);
        const response = await fetch("https://85.192.30.34:3000/api/update-discounts", {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
  ...
  const updatePriceListDiscount = async () => {
    ...
    const response = await fetch("https://85.192.30.34:3000/api/update-discount-Pricelist", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const addNewOrder = async (newOrder: Order) => {
    try {
        const response = await fetch("https://85.192.30.34:3000/api/orders", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
