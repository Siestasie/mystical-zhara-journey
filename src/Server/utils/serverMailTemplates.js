 ...
 * @param {string} verificationLink - ссылка для подтверждения
 * @returns {string} HTML-строка с шаблоном письма
 */
const generateVerificationMail = (userName, verificationLink) => {
    return `
    <!DOCTYPE html>
    <html>
      <head>
   ...
   * @returns {string} HTML-строка с шаблоном письма
   */
  const generatePasswordResetMail = (userName, resetLink) => {
    return `
    <!DOCTYPE html>
    <html>
      <head>
