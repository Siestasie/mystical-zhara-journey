...
const Blog = () => {
  ...
  const { data: posts = [] } = useQuery<BlogPost[]>({
    queryKey: ['blog-posts'],
    queryFn: async () => {
      const response = await fetch('https://85.192.30.34:3000/api/blog-posts');
  ...
  const deletePostMutation = useMutation({
    mutationFn: async (postId: number) => {
      const response = await fetch(`https://85.192.30.34:3000/api/blog-posts/${postId}`, {
        method: 'DELETE',
      });
              ...
              <CardContent className="space-y-4">
                {post.image && (
                  <div className="aspect-video relative overflow-hidden rounded-lg">
                    <img 
                      src={`https://85.192.30.34:3000${post.image}`}
                      alt={post.title}
