...
export function AuthDialogs({ isLoginOpen, isRegisterOpen, isResetPasswordOpen, onLoginClose, onRegisterClose, onResetPasswordClose}: AuthDialogsProps) {
  ...
  const onLogin = async (data: LoginFormData) => {
    setIsLoading(true);
    try {
      const response = await fetch('https://85.192.30.34:3000/api/login', {
        method: 'POST',
        headers: {
  ...
  const onRegister = async (data: RegisterFormData) => {
    setIsLoading(true);
    try {
      const response = await fetch('https://85.192.30.34:3000/api/register', {
        method: 'POST',
  ...
  const handleResendVerification = async () => {
    ...
    try {
      setCanResend(false);
      const response = await fetch('https://85.192.30.34:3000/api/resend-verification', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const onResetPassword = async (data: { email: string }) => {
    setIsLoading(true);
    try {
      const response = await fetch('https://85.192.30.34:3000/api/request-password-reset', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      });
